...
*/
#define SP_LOG_AI_TASK_EXECUTE(CheckSelected, Str, ...)\
	SP_IF_AI_TASK_EXECUTE(CheckSelected)\
...
*/
#define SP_LOG_AI_TASK_TICK(CheckSelected, Str, ...)\
	SP_IF_AI_TASK_TICK(CheckSelected)\
	SP_LOG_SCREEN_FULL(Display, -1, FColor::Magenta,  USP_Settings::GetDebugScreenDisplayTime(), "%s: " Str, *GetName(), ##__VA_ARGS__)

#else

#define SP_IF_AI_TASK_EXECUTE(...)
#define SP_IF_AI_TASK_TICK(...)

#define SP_LOG_AI_TASK_EXECUTE(...)
#define SP_LOG_AI_TASK_TICK(...)
